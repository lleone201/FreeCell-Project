...
public class CardStack extends JLayeredPane
{
	...
	public CardStack(int t, int w)
	{
		...
		offset = 1;				//The first non-blank card in a pile is directly on top of the blank card
		stackType = t;
		width = w;
		if(stackType == 1)
			capacity = 1;
	...
	public void addCard(Card c)
	{
		...
		this.add(c, 1, 0);
 		if(stackType == 0)
			offset = 20;	//After the first non-blank card is added, the offset is updated
		resize();
	}
	...
	public void resize()
	{
		...
		setLayer(pile.get(pile.size() - 1), DRAG_LAYER, 0);
	}
